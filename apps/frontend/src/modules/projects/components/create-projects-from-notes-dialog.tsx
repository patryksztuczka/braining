...
export function CreateProjectsFromNotesDialog({
  ...
  function toggleAll() {
    if (allAvailableSelected) {
      setSelected((prev) => {
        const next = new Set(prev);
        for (const resource of filtered) next.delete(resource.prefix);
        return next;
      });
    } else {
      setSelected((prev) => {
        const next = new Set(prev);
        for (const resource of filtered) next.add(resource.prefix);
        return next;
      });
    }
  }
      ...
      >
        ...
        {filtered.length > 1 && !isLoading && !isError && (
          <div className="shrink-0 px-5 pb-1">
            ...
            >
              <Checkbox checked={allAvailableSelected} />
